...
internal object AdaptiveMarketScoring {
 ...
 private fun robust(a:List<Pair<AdaptiveMarketSample,Double>>):List<Pair<AdaptiveMarketSample,Double>> { val v=a.map{it.first.ratePerKm}.sorted(); val m=v[v.size/2]; val d=v.map{abs(it-m)}.sorted()[v.size/2]; return if(d==0.0)a else a.filter{abs(it.first.ratePerKm-m)<=6*d} }
}
