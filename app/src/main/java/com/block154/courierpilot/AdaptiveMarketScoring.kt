...
internal data class MarketObservation(val ratePerKm: Double, val capturedAtEpochMs: Long, val cityKey: String, val currencyCode: String, val platform: String, val installationId: String? = null)
...
internal object AdaptiveMarketScoring {
 ...
 fun profile(os:List<MarketObservation>,now:Long,source:MarketProfileSource=MarketProfileSource.PERSONAL):AdaptiveMarketProfile? { val a=os.mapNotNull{val w=recencyWeight((now-it.capturedAtEpochMs)/86400000.0); if(w>0&&it.ratePerKm.isFinite()&&it.ratePerKm>0)it to w else null}; if(a.isEmpty())return null; val s=robust(a); val v=s.map{it.first.ratePerKm}; val w=s.map{it.second}; return AdaptiveMarketProfile(v.size,effectiveSampleSize(w),q(v,w,.5),q(v,w,.15),q(v,w,.35),q(v,w,.65),q(v,w,.85),when{v.size<5->MarketConfidence.NOT_READY;v.size<10->MarketConfidence.LOW;v.size<25->MarketConfidence.MEDIUM;else->MarketConfidence.HIGH},source) }
 fun score(rate:Double,os:List<MarketObservation>,now:Long,city:AdaptiveMarketProfile?=null):MarketScore { val p=profile(os,now); val x=p?:city?:return MarketScore(OfferDecisionBand.UNKNOWN,null,MarketConfidence.NOT_READY,MarketProfileSource.LEARNING,null); val pct=rank(rate,x); return MarketScore(when{pct<.15->OfferDecisionBand.TERRIBLE;pct<.35->OfferDecisionBand.BAD;pct<.65->OfferDecisionBand.OK;pct<.85->OfferDecisionBand.GOOD;else->OfferDecisionBand.FIRE},pct,x.confidence,if(p!=null&&city!=null)MarketProfileSource.PERSONAL_AND_CITY else x.source,x) }
 ...
 private fun robust(a:List<Pair<MarketObservation,Double>>):List<Pair<MarketObservation,Double>> { val v=a.map{it.first.ratePerKm}.sorted(); val m=v[v.size/2]; val d=v.map{abs(it-m)}.sorted()[v.size/2]; return if(d==0.0)a else a.filter{abs(it.first.ratePerKm-m)<=6*d} }
}
