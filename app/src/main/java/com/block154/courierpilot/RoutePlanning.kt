 ...
 * Missing addresses remain explicit unresolved waypoints so later geocoding/Bolt recovery can fill
...
/**
 * Research-only side-by-side executor. It intentionally does not choose a winner: real Vilnius
 * validation comes first, and one profile failing must not discard the other candidate.
 */
