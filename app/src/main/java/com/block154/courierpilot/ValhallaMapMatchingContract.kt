...
/**
 * Pure JSON contract for Valhalla /trace_attributes. The official API exposes edge.id/way_id and
 * one matched point per input location when map_snap is used, which is exactly the evidence needed
 * for later personal traversal statistics.
 */
internal object ValhallaMapMatchingContract {
    fun buildTraceAttributesPayload(samples: List<GpsTraceSample>, profile: RouteProfile): String {
        ...
        samples.forEach { sample ->
            val relativeSeconds = (sample.timestampMillis - firstTimestamp) / 1000.0
            shape.put(
                JSONObject()
                    .put("lat", sample.point.latitude)
                    .put("lon", sample.point.longitude)
                    .put("time", relativeSeconds)
            )
        }
        ...
        val attributes = JSONArray().apply {
            ...
            put("matched.distance_from_trace_point")
            put("shape")
        }

        return JSONObject()
            .put("shape", shape)
            .put("costing", when (profile) {
    ...
    fun parseTraceAttributesResponse(json: String): MapMatchedTrace {
        ...
        val matchedPoints = buildList {
            for (index in 0 until pointsJson.length()) {
                ...
                val edgeIndex = point.optInt("edge_index", -1).takeIf { it >= 0 }
                val distance = point.optDouble("distance_from_trace_point", Double.NaN)
                    .takeIf { it.isFinite() }
                add(
                    MatchedTracePoint(
                        ...
                        edgeIndex = edgeIndex,
                        distanceFromTracePointMeters = distance,
                    )
                )
            }
        }

        return MapMatchedTrace(
            edges = edges,
            matchedPoints = matchedPoints,
            encodedShape = root.optString("shape").takeIf(String::isNotBlank),
        )
    }
    ...
    private fun representativeAccuracy(samples: List<GpsTraceSample>): Double {
        val accuracies = samples.mapNotNull { it.accuracyMeters?.toDouble() }
            ...
            .sorted()
        if (accuracies.isEmpty()) return 8.0
        return accuracies[accuracies.size / 2].coerceIn(2.0, 50.0)
    }
}
