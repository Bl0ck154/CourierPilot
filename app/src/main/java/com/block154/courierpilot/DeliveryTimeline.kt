...
internal object DeliveryTimelineAnalyzer {
    fun analyze(events: List<DeliveryTimelineEvent>): DeliveryTimelineMetrics {
        ...
        val pickupWaits = ordered
            ...
            .mapNotNull { arrival ->
                val pickup = ordered.firstOrNull {
                    it.type == DeliveryEventType.PICKED_UP && it.stopKey == arrival.stopKey && it.timestampMillis >= arrival.timestampMillis
                } ?: return@mapNotNull null
                PickupWaitMetric(arrival.stopKey!!, arrival.timestampMillis, pickup.timestampMillis)
            }
        ...
        return DeliveryTimelineMetrics(accepted, completed, total, pickupWaits, cancelled)
    }
}
